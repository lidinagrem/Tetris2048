...
public class Tetromino {
   ...
   public Tetromino(char type) {
      ...
      if (type == 'I') {
         ...
         occupiedTiles[0] = new Point(1.0, 0.0); // (column_index, row_index)
         occupiedTiles[1] = new Point(1.0, 1.0);
         occupiedTiles[2] = new Point(1.0, 2.0);
         occupiedTiles[3] = new Point(1.0, 3.0);
      } else if (type == 'O') {
         n = 2; // n = number of rows = number of columns in the tile matrix
         // shape of the tetromino O in its initial orientation
         occupiedTiles[0] = new Point(0.0, 0.0); // (column_index, row_index)
         occupiedTiles[1] = new Point(1.0, 0.0);
         occupiedTiles[2] = new Point(0.0, 1.0);
         occupiedTiles[3] = new Point(1.0, 1.0);
      } else if (type == 'Z') {
         n = 3; // n = number of rows = number of columns in the tile matrix
         // shape of the tetromino Z in its initial orientation
         occupiedTiles[0] = new Point(0.0, 2.0); // (column_index, row_index)
         occupiedTiles[1] = new Point(1.0, 2.0);
         occupiedTiles[2] = new Point(1.0, 1.0);
         occupiedTiles[3] = new Point(2.0, 1.0);
      }   else if (type == 'S') {
          n = 3; // n = number of rows = number of columns in the tile matrix
          // shape of the tetromino O in its initial orientation
          occupiedTiles[0] = new Point(0.0, 1.0); // (column_index, row_index)
          occupiedTiles[1] = new Point(1.0, 1.0);
          occupiedTiles[2] = new Point(1.0, 2.0);
          occupiedTiles[3] = new Point(2.0, 2.0);
       } else if (type == 'T') {
           n = 3; // n = number of rows = number of columns in the tile matrix
           // shape of the tetromino O in its initial orientation
           occupiedTiles[0] = new Point(0.0, 1.0); // (column_index, row_index)
           occupiedTiles[1] = new Point(1.0, 1.0);
           occupiedTiles[2] = new Point(1.0, 2.0);
           occupiedTiles[3] = new Point(1.0, 0.0);
        } else if (type == 'L') {
            n = 3; // n = number of rows = number of columns in the tile matrix
            // shape of the tetromino O in its initial orientation
            occupiedTiles[0] = new Point(1.0, 0.0); // (column_index, row_index)
            occupiedTiles[1] = new Point(0.0, 0.0);
            occupiedTiles[2] = new Point(0.0, 1.0);
            occupiedTiles[3] = new Point(0.0, 2.0);
         } else { //type == 'J'
             n = 3; // n = number of rows = number of columns in the tile matrix
             // shape of the tetromino O in its initial orientation
             occupiedTiles[0] = new Point(0.0, 0.0); // (column_index, row_index)
             occupiedTiles[1] = new Point(1.0, 0.0);
             occupiedTiles[2] = new Point(1.0, 1.0);
             occupiedTiles[3] = new Point(1.0, 2.0);
          }
      ...
      for (Point tilePosition : occupiedTiles) { // using a for-each loop
         int colIndex = (int) tilePosition.getX(), rowIndex = (int) tilePosition.getY();
   ...
   public void createMinBoundedTileMatrix() {
      ...
      int blcX = (int) bottomLeftCell.getX(), blcY = (int) bottomLeftCell.getY();
   ...
   public void rotateTetromino(GameGrid gameGrid, boolean direction){

      // checks if the tetromino can be rotated
      if(canBeRotated(gameGrid)){
         System.out.println("Rotating the piece"); // Prints the statement
         // Gets the number of rows and columns of the tile matrix of the tetromino
         int rows = tileMatrix.length;
         int cols = tileMatrix[0].length;
         // Creates new tile matrix with swapped rows and columns
         Tile [][]  newMatrix = new Tile[cols][rows];
         
         // Rotates the tile matrix clockwise if the direction is true
         if(direction){
            for(int row = 0; row < rows; row++){
               for(int col = 0; col < cols; col++){
                  newMatrix[col][rows - 1 - row] = tileMatrix[row][col];
               }
            }
         } 
         // Rotates the tile matrix counterclockwise if the direction is false
         else {
            for (int row = 0; row < rows; row++) {
               for (int col = 0; col < cols; col++) {
                   newMatrix[cols - 1 - col][row] = tileMatrix[row][col];
               }
           }
         }
       // Sets the new tile matrix as the tile matrix of the tetromino
       tileMatrix = newMatrix;
      }
   }
   
   // A method to check if a tetromino can be rotated
   public boolean canBeRotated(GameGrid gameGrid){

      // Initialize count variable and retrieve the number of tiles in the current tetromino
      int count = 0;
      int number = gameGrid.currentTetromino.n;

      // Loop through each cell in the current tetromino matrix
      for(int row = 0; row < number; row++){
         for(int col = 0; col < number; col++){
            // Gets the cell position of the cell in the gamegrid
            Point currentPos = getCellPosition(row, col);

            // Returns false if the cell is ouside the game grid (the piece can't rotate)
            if (currentPos.x < 0 || currentPos.x >= gridWidth || currentPos.y < 0 || currentPos.y >= gridHeight) {
               return false;
            }
               else{
                  // If the cell is not null increments the count to check if any cell is overlapping after the rotation
                  if(gameGrid.tileMatrix[(int) currentPos.y][(int) currentPos.x] != null){
                     count++;
                  }
               }
         }
      }
         // If any cell overlaps after the rotation returns false (the piece can't rotate)
         if(count > 0){
            return false;
         }
         // Returns true if there are no overlapping and the piece will be in the gamegrid.
         return true;
   }
}